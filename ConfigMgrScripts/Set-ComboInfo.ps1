...
#Run These during OSD
    ...
    {
        if($TSEnv.Value('SMSTS_FinishUpdateTime'))
            {
            $DifferenceUpdates = ([datetime]$TSEnv.Value('SMSTS_FinishUpdateTime')) - ([datetime]$TSEnv.Value('SMSTS_StartUpdateTime')) 
            $DifferenceUpdates = [math]::Round($DifferenceUpdates.TotalMinutes)
            New-Variable -Name "$($AttributePrefix)OSD_UpdateTime" -Value "$DifferenceUpdates"
            }
        if($TSEnv.Value('SMSTS_FinishAppTime'))
            {
            $DifferenceApps = ([datetime]$TSEnv.Value('SMSTS_FinishAppsTime')) - ([datetime]$TSEnv.Value('SMSTS_StartAppsTime')) 
            $DifferenceApps = [math]::Round($DifferenceApps.TotalMinutes)
            New-Variable -Name "$($AttributePrefix)OSD_AppsTime" -Value "$DifferenceApps"
            }
...
#	    New-Variable -Name "$($AttributePrefix)OSD_DMLocation" -Value $tsenv.Value("SMSTS_DMLocation")
		...
		New-Variable -Name "$($AttributePrefix)OSDDomainName" -Value $tsenv.Value("OSDDomainName")
		New-Variable -Name "$($AttributePrefix)OSDDomainOUName" -Value $tsenv.Value("OSDDomainOUName")
		New-Variable -Name "$($AttributePrefix)OSDBranding" -Value $tsenv.Value("OSDBranding")
		New-Variable -Name "$($AttributePrefix)OSDComputerName" -Value $tsenv.Value("OSDComputerName")
        if ($tsenv.Value("_SMSTSUserStarted"))
            {
            New-Variable -Name "$($AttributePrefix)OSD_UserInitiated" -Value $tsenv.Value("_SMSTSUserStarted")
            #I don't know why this happens if you start the TS in Windows and "Reimage" vs BareMetal, but it reverses the Start & Finish Time... this accounts for that
            $Difference = ([datetime]$TSEnv.Value('SMSTS_StartTSTime')) - ([datetime]$TSEnv.Value('SMSTS_FinishTSTime')) 
            $Difference = [math]::Round($Difference.TotalMinutes)
            New-Variable -Name "$($AttributePrefix)OSD_TSRunTime" -Value "$Difference"
            }
        Else{New-Variable -Name "$($AttributePrefix)OSD_TSRunTime" -Value "$Difference"}
    }
